...
async function connectDatabaseWithRetry(connectDatabaseFn, retries = MAX_INIT_RETRIES) {
    let lastError = null;
...
async function getApp(createAppFn, connectDatabaseFn) {
    ...
    if (initializationError) {
        const errorAge = Date.now() - initializationError.timestamp;
...
async function handler(req, res) {
    ...
    try {
        if (!createAppModule) {
            ...
            try {
                createAppModule = await Promise.resolve().then(() => __importStar(require('../dist/app.js')));
                ...
                try {
                    ...
                    try {
                        ...
                        try {
                            createAppModule = await Promise.resolve().then(() => __importStar(require('../src/app.js')));
        ...
        if (!configModule) {
            ...
            // Dynamic imports from dist folder (runtime only, not available at compile time)
            try {
                configModule = await Promise.resolve().then(() => __importStar(require('../dist/config/index.js')));
            }
            catch (e1) {
                try {
                    configModule = await Promise.resolve().then(() => __importStar(require('./dist/config/index.js')));
